...
class EvidenceVector(BaseModel):
    ...
    mac: str = Field(default="", description="Target IEEE 802 MAC address")
    ttl: int = Field(default=0, ge=0, le=255, description="ICMP / IP Time-To-Live")
    open_ports: List[int] = Field(default_factory=list, description="Observed open TCP/UDP ports")
    banners: Dict[Any, Any] = Field(default_factory=dict, description="Captured service banners")
    dpi_data: Dict[str, Any] = Field(default_factory=dict, description="L7 Deep Packet Inspection metadata")
    subnet_cidr: str = Field(default="", description="Subnet CIDR notation")
    vendor_hint: str = Field(default="", description="Inferred or declared hardware vendor")
    hostname_hint: str = Field(default="", description="Inferred network hostname")
    net_flight_us: Optional[float] = Field(default=None, description="Calibrated net microsecond flight time")
    estimated_distance_m: Optional[float] = Field(default=None, description="Calculated conductor distance in meters")
    is_trunk: bool = Field(default=False, description="Flag indicating 802.1Q trunk uplink status")
    port_id: Optional[str] = Field(default=None, description="Switchport identifier")
    ...
    def validate_ports(cls, ports: List[int]) -> List[int]:
        for p in ports:
            if not (0 <= p <= 65535):
                raise ValueError(f"Port number {p} out of valid TCP/UDP range [0, 65535]")
        return ports
...
class HypothesisScore(BaseModel):
    ...
    archetype: str = Field(..., description="Inferred hardware or software archetype")
    confidence: float = Field(default=0.0, ge=0.0, le=1.0, description="Archetype probability score [0.0, 1.0]")
    evidence_factors: List[str] = Field(default_factory=list, description="Rationales contributing to confidence score")

    def __init__(
        self,
        archetype: str = "",
        confidence: Optional[float] = None,
        evidence_factors: Optional[List[str]] = None,
        **data: Any,
    ):
        if archetype and "archetype" not in data:
            data["archetype"] = archetype
        if confidence is not None and "confidence" not in data:
            data["confidence"] = confidence
        if evidence_factors is not None and "evidence_factors" not in data:
            data["evidence_factors"] = evidence_factors
        super().__init__(**data)
...
DEFAULT_PORT_RESOLVERS: Dict[str, Callable[[List[int]], int]] = {
    "winrm": lambda ports: next((p for p in [5985, 5986, 25985] if p in ports), 5985),
    "rpc": lambda ports: 135 if 135 in ports else 20135,
    "smb": lambda ports: 445 if 445 in ports else (139 if 139 in ports else 20445),
    "rdp": lambda ports: 3389,
    "sip": lambda ports: 5060 if 5060 in ports else 5061,
    "onvif": lambda ports: next((p for p in [80, 443, 8080, 28082, 28092] if p in ports), 80),
    "rtsp": lambda ports: next((p for p in [554, 8554, 25554, 25555, 37777] if p in ports), 554),
    "tls": lambda ports: next((p for p in [443, 8443, 28080, 28081] if p in ports), 443),
    "http_web": lambda ports: next((p for p in [80, 8080, 8000] if p in ports), 80),
    "ssdp": lambda ports: 1900,
    "msp": lambda ports: next((p for p in [3001, 23001] if p in ports), 3001),
    "modbus": lambda ports: next((p for p in [502, 20502, 20504] if p in ports), 502),
}
...
class AdaptiveDiscoveryOrchestrator:
    ...
    Decoupled from direct deep_prober imports via injectable prober dispatch map.
    """

    PORT_RESOLVERS = DEFAULT_PORT_RESOLVERS

    def __init__(
        self,
        prober_dispatcher: Optional[Dict[str, Callable[..., Dict[str, Any]]]] = None,
    ):
        self.memory = SubnetAdaptiveMemory()
        if prober_dispatcher is not None:
            self.prober_dispatcher: Dict[str, Callable[..., Dict[str, Any]]] = dict(prober_dispatcher)
        else:
            self.prober_dispatcher = self.get_default_prober_dispatcher()
        self.metrics = {
    ...
    @staticmethod
    def get_default_prober_dispatcher() -> Dict[str, Callable[..., Dict[str, Any]]]:
        """
        Lazily constructs the default prober dispatcher map without top-level module coupling.
        """
        try:
            ...
            return {
                "winrm": WinRmProber.probe_winrm,
                "rpc": RpcProber.probe_rpc,
                "smb": SmbProber.probe_smb,
                "rdp": RdpProber.probe_rdp,
                "sip": SipProber.probe_sip,
                "onvif": WebDeepProber.probe_onvif_soap,
                "rtsp": RtspProber.probe_rtsp,
                "tls": WebDeepProber.inspect_tls_cert,
                "http_web": WebDeepProber.probe_upnp_description,
                "ssdp": SsdpProber.probe_ssdp,
                "msp": MercuryMspProber.probe_msp,
                "modbus": ModbusProber.probe_modbus,
            }
        except ImportError:
            return {}

    def register_prober(self, probe_name: str, handler: Callable[..., Dict[str, Any]]) -> None:
        """Dynamically registers or overrides a prober dispatcher handler."""
        self.prober_dispatcher[probe_name] = handler

    def _dispatch_probe(
        self, probe_name: str, ev: EvidenceVector, timeout: float
    ) -> Dict[str, Any]:
        """
        Executes a targeted probe using the registered dispatcher.
        Handles flexible callable signatures: (ip, port, timeout), (ip, timeout), or (ev, timeout).
        """
        handler = self.prober_dispatcher.get(probe_name)
        if not handler:
            return {}

        port_resolver = self.PORT_RESOLVERS.get(probe_name, lambda ports: 0)
        port = port_resolver(ev.open_ports)

        try:
            import inspect
            sig = inspect.signature(handler)
            params = list(sig.parameters.keys())
            if len(params) == 2 and ("ev" in params or "evidence" in params):
                res = handler(ev, timeout=timeout)
            elif len(params) == 2 and "port" not in params:
                res = handler(ev.ip, timeout=timeout)
            else:
                res = handler(ev.ip, port, timeout=timeout)
        except TypeError:
            try:
                res = handler(ev.ip, port, timeout=timeout)
            except TypeError:
        ...
        # Spatial resolution for Mercury MSP peripherals if present
        if probe_name == "msp" and res.get("peripherals"):
            try:
                from aetheris.discovery.mercury_spatial_resolver import MercurySpatialResolver
                resolved_periphs = []
                for periph in res["peripherals"]:
                ...
                res["peripherals"] = resolved_periphs
            except Exception:
                pass
    ...
    def execute_adaptive_sweep(self, ev: EvidenceVector, timeout: float = 0.6) -> Dict[str, Any]:
        ...
        for probe_name in probe_queue:
            ...
            probe_res: Dict[str, Any] = self._dispatch_probe(probe_name, ev, timeout)

...
class NetworkClassifierEngine:
    ...
    def ingest_telemetry_event(self, channel_key: Tuple[str, str], timestamp: float, size: int, op_code: str) -> Dict[str, Any]:
        """
        ...
        if buf.last_ts is not None:
            dt = max(1e-7, timestamp - buf.last_ts)
            buf.iat.append(dt)
        buf.last_ts = timestamp

        buf.sizes.append(size)
        buf.states.append(op_code)

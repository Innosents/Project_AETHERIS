    ...
    def audit_device(cls, device_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 80 in open_ports:
            ...
            findings.append({
                ...
                "code": "UNENCRYPTED_HTTP_MANAGEMENT",
        ...
        if 5060 in open_ports or dev_type in ("voip_phone", "voip_pbx", "ip_phone", "pbx"):
            if 5060 in open_ports:
                ...
                findings.append({
                    ...
                    "code": "UNENCRYPTED_SIP_SIGNALING",
        ...
        if 502 in open_ports or dev_type in ("plc", "hmi"):
            ...
            findings.append({
                ...
                "code": "UNAUTHENTICATED_MODBUS_EXPOSED",
        ...
        if matched_profile_key and matched_profile_key in FACTORY_POSTURE_PROFILES:
            ...
            if 80 in open_ports or 443 in open_ports or 22 in open_ports or 23 in open_ports:
                ...
                findings.append({
                    ...
                    "code": "FACTORY_DEFAULT_CONFIGURATION_RISK",
        ...
        for oid in snmp_oids:
            ...
            if "public" in val_str or "private" in val_str:
                ...
                findings.append({
                    ...
                    "code": "DEFAULT_SNMP_COMMUNITY_DETECTED",
        ...
        if has_tap_anomaly:
            ...
            findings.append({
                "severity": "HIGH",
                "title": "PHYSICAL_LAYER_INLINE_TAP_SUSPECTED",
                "code": "PHYSICAL_LAYER_INLINE_TAP_SUSPECTED",
                "advisory_id": "PHYSICAL_LAYER_INLINE_TAP_SUSPECTED",
                "description": "Conductor DC drop distance diverges from physical serial baud-rate expected latency by more than 35%, indicating spliced hardware, parasitic inline tap (e.g. ESPKey, BLE sniffer), or severe terminal oxidation."
            })
        ...
        if has_bus_overextension:
            ...
            findings.append({
                "severity": "MEDIUM",
                "title": "ACCESS_PERIPHERAL_BUS_OVER_EXTENSION",
                "code": "ACCESS_PERIPHERAL_BUS_OVER_EXTENSION",
                "advisory_id": "ACCESS_PERIPHERAL_BUS_OVER_EXTENSION",
                "description": "Access peripheral physical conductor length exceeds standard 152.4m (500ft) threshold for low-voltage serial sub-buses, causing transmission degradation and potential out-of-perimeter line exposure."
            })
        ...
        if has_inductive_tamper:
            ...
            findings.append({
                "severity": "HIGH",
                "title": "SOLENOID_INDUCTIVE_TAMPER_OR_FLYBACK_ABSENT",
                "code": "SOLENOID_INDUCTIVE_TAMPER_OR_FLYBACK_ABSENT",
                "advisory_id": "SOLENOID_INDUCTIVE_TAMPER_OR_FLYBACK_ABSENT",
                "description": "Observed 3 or more transient voltage drop rejections within 60 seconds on inductive access peripheral, indicating physical strike relay manipulation, actuator buzzing, or missing reverse-biased flyback clamping diode."
            })
        ...
        if tcp_flight_us is not None and float(tcp_flight_us) > 2000.0 and is_controller_node:
            ...
            findings.append({
                "severity": "CRITICAL",
                "title": "ACCESS_CONTROLLER_SPATIAL_IMPERSONATION",
                "code": "ACCESS_CONTROLLER_SPATIAL_IMPERSONATION",
                "advisory_id": "ACCESS_CONTROLLER_SPATIAL_IMPERSONATION",
                "description": f"Access controller TCP flight time ({float(tcp_flight_us):.1f} microseconds) exceeds the 2000 microsecond physical local LAN limit, indicating an off-path rogue proxy, WAN overlay redirection, or spatial identity impersonation."
            })

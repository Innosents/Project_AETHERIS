...
class SpatialEstimator:
    ...
    def calibrate_anchor(self, archetype: str, observed_rtt_samples: List[float], true_distance_m: float):
    ...
    def estimate_node_distance(self, rtt_samples: List[float], archetype: str) -> SweepEstimate:
        if not rtt_samples:
            return SweepEstimate(0.0, float("inf"), 0.0, 0.0)
        ...
        if net_flight_time <= 0:
            return SweepEstimate(0.5, 10.0, 75.0, 0.0)

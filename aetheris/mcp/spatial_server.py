...
from aetheris.core.parsers.sanitization import sanitize_prober_payload, clean_ascii_string
...
class TelemetryIngestPayload(BaseModel):
    ...
    anchor_trust_state: Optional[str] = None
...
def render_visualizer():
    """Serves the standalone Cytoscape spatial visualizer."""
    return HTMLResponse(content=HTML_DASHBOARD, status_code=200)


@app.get("/topology_audit.json", tags=["Topology"])
async def get_topology_audit():
    """Serves the generated topology_audit.json directly for Cytoscape.js rendering."""
    root_dir = Path(__file__).resolve().parent.parent.parent
    audit_file = root_dir / "topology_audit.json"
    if not audit_file.exists():
        audit_file = Path("topology_audit.json")
...
@app.get("/app.js", tags=["Static"])
async def get_app_js():
    """Serves the frontend app.js script."""
    root_dir = Path(__file__).resolve().parent.parent.parent
    js_file = root_dir / "aetheris" / "web" / "app.js"
    if not js_file.exists():
        js_file = Path("aetheris/web/app.js")
    if js_file.exists():
        return Response(content=js_file.read_text(encoding="utf-8"), media_type="application/javascript")
    return Response(content="// app.js not found", status_code=404, media_type="application/javascript")
...
async def get_graph():
    ...
    styling classes:
      - .anchor: Gold border, glowing halo (trusted physical anchors only).
      - .anchor_disqualified: Hazard pulsing halo (disqualified wireless / unbound anchors).
    ...
    elements.append({
        "data": {
            ...
            "archetype": "SWITCH",
            "is_anchor": False,
            "anchor_trust_state": "CORE_SWITCH"
        },
        "classes": "switch"
    })
    ...
    for n in latest_telemetry["nodes"]:
        ...
        anchor_trust_state = n.get("anchor_trust_state") or ("TRUSTED_PHYSICAL_ANCHOR" if is_anchor else "UNQUALIFIED_ENDPOINT")
        ...
        classes = []
        if is_anchor and anchor_trust_state == "TRUSTED_PHYSICAL_ANCHOR":
            classes.append("anchor")
        elif "DISQUALIFIED" in anchor_trust_state:
            classes.append("anchor_disqualified")
    ...
    for n in latest_telemetry["nodes"]:
        ...
        if is_anchor and anchor_trust_state == "TRUSTED_PHYSICAL_ANCHOR":
            ...
            edge_classes.append("anchor_disqualified")
...
async def ingest_telemetry_endpoint(request: Request):
    ...
    Parses and normalizes:
      - Node: ip, mac, hostname, device_type, archetype, os_profile, confidence, is_anchor, anchor_trust_state.
    ...
    medium_in = data.get("medium") or node_props.get("medium")
    ...
    # Evaluate anchor validity via 4-tier AnchorGuard
    from aetheris.core.anchor_guard import AnchorGuard
    claimed_anchor = bool(
        data.get("is_anchor", False)
        or node_props.get("is_anchor", False)
        or (data.get("anchor_trust_state") and "DISQUALIFIED" in str(data.get("anchor_trust_state")))
        or (node_props.get("anchor_trust_state") and "DISQUALIFIED" in str(node_props.get("anchor_trust_state")))
    )
    guard_res = AnchorGuard.evaluate_anchor_candidate({
        "is_anchor": claimed_anchor,
        "medium": medium_in,
        "edge_type": data.get("edge_type"),
        "switchport": port,
        "device_type": device_type,
        "archetype": archetype,
        "device_hint": hostname,
        "hostname": hostname,
        "jitter_ns": jitter_ns,
    })
    is_anchor = guard_res["is_anchor"]
    anchor_trust_state = data.get("anchor_trust_state") or guard_res["anchor_trust_state"]
    if not is_anchor and "DISQUALIFIED" in guard_res["anchor_trust_state"]:
        anchor_trust_state = guard_res["anchor_trust_state"]
    disqualification_reason = data.get("disqualification_reason") or guard_res["disqualification_reason"]
    medium = guard_res["medium"]
    edge_type = data.get("edge_type") or guard_res["edge_type"]
    if not is_anchor and edge_type == "ETHERNET_ANCHOR":
        edge_type = guard_res["edge_type"]
...
def _hydrate_initial_topology() -> None:
    """Pre-populates latest_telemetry from topology_audit.json and spatial_ledger.db if empty."""
    if latest_telemetry.get("nodes"):
        return
    import sqlite3
    from aetheris.core.anchor_guard import AnchorGuard
    root_dir = Path(__file__).resolve().parent.parent.parent
    ...
    try:
        ...
        db_file = root_dir / "spatial_ledger.db"
        if not db_file.exists():
            db_file = Path("spatial_ledger.db")
        mv_data = {}
        if db_file.exists():
            try:
                with sqlite3.connect(str(db_file)) as conn:
                    conn.row_factory = sqlite3.Row
                    for r in conn.execute("SELECT ip, os_profile, inferred_link_speed, is_throttled FROM port1_multivector_analysis").fetchall():
                        mv_data[r["ip"]] = dict(r)
            except Exception:
                pass

        for ep in audit.get("converged_endpoints", []):
            mac_clean = ep["mac"].upper()
            port_meta = mac_to_port.get(mac_clean, {})
            gt_meta = gt_pins.get(mac_clean)

            requested_anchor = (ep.get("ip") == "192.168.1.86" or gt_meta is not None)
            port_name = port_meta.get("port_id", "WLAN")
            parent_switch = "Actiontec-Q6000" if (port_name == "Port 1" and mac_clean != "10:78:5B:3D:08:80") else "Gateway-Core"

            mv = mv_data.get(ep["ip"], {})
            os_prof = mv.get("os_profile") or ep.get("archetype", "GENERIC_HOST")
            link_spd = mv.get("inferred_link_speed")
            medium_str = f"Copper ({link_spd})" if link_spd else ep.get("medium", "Unknown")

            dist = float(ep.get("converged_distance_m", 0.0))
            var = float(ep.get("variance_m2", 0.0))
            conf = float(ep.get("confidence_pct", 95.0))
            node_id = f"host_{ep['ip'].replace('.', '_')}"
            hostname = gt_meta.get("canonical_name") if gt_meta else port_meta.get("hostname", ep["ip"])

            # 4-tier AnchorGuard qualification
            guard_res = AnchorGuard.evaluate_anchor_candidate({
                "is_anchor": requested_anchor,
                "medium": medium_str,
                "switchport": port_name,
                "archetype": os_prof,
                "device_type": ep.get("device_type", "endpoint"),
                "jitter_ns": float(ep.get("jitter_ns", 0.0)),
            })
            is_anchor = guard_res["is_anchor"]
            anchor_trust_state = guard_res["anchor_trust_state"]
            disqualification_reason = guard_res["disqualification_reason"]
            edge_type = guard_res["edge_type"]
            medium_str = guard_res["medium"]

            label_str = f"{dist:.1f}m ({conf:.0f}%)" if edge_type != "WIRELESS_AIRLINK" else (f"~{dist:.1f}m ({conf:.0f}%)" if dist > 0 else "AirLink")
            ...
            latest_telemetry["nodes"].append(node_rec)

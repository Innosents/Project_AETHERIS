"""
Project AETHERIS - Connectionless LDAP (CLDAP) Active Directory Discovery Engine
MS-ADTS 6.3.5 / RFC 1798 UDP Port 389 Ping Prober

Sends a zero-credential connectionless LDAP searchRequest querying the 'netlogon'
attribute. Dissects the authoritative NETLOGON_SAM_LOGON_RESPONSE_EX binary
structure to discover:
  - Active Directory Forest and Domain FQDN
  - Domain Controller Hostname and NetBIOS Computer Name
  - Primary Domain Controller (PDC), Global Catalog (GC), and KDC capabilities
  - Domain GUID and AD Site topology (DC Site, Client Site)
  - Microsecond network turnaround latency (tau_turnaround)
"""
...
def build_cldap_netlogon_ping(domain: str = "") -> bytes:
    ...
    base_dn = _ber_tag(0x04, b"")                   # Base DN ""
    scope = _ber_tag(0x0a, b"\x00")                     # baseObject (0)
    deref = _ber_tag(0x0a, b"\x00")                     # neverDerefAliases (0)
    size_limit = _ber_tag(0x02, b"\x00")                # 0 (no limit)
    time_limit = _ber_tag(0x02, b"\x00")                # 0 (no limit)
    types_only = _ber_tag(0x01, b"\x00")                # FALSE

    # NtVer Equality Filter: (NtVer=\x06\x00\x00\x20)
    # 0x20000006 = NETLOGON_NT_VERSION_5EX | NETLOGON_NT_VERSION_WITH_CLOSEST_SITE

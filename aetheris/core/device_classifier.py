"""
Project AETHERIS - Device Classifier
Executes deterministic O(1) MAC prefix matching against hardware OUI registries to infer endpoint operating profiles and device archetypes. Dynamically calibrates transmission line Nominal Velocity of Propagation
(NVP in [0.68, 0.72]) to bound electromagnetic propagation velocities for physical-layer spatial deconvolution.
"""
...
class DeviceClassifier(DeviceClassifierPort):
    ...
    def classify_oui(cls, mac: str) -> Optional[OuiClassificationResult]:
        ...
        if clean_mac in cls.OUI_DB:
            return OuiClassificationResult.model_validate(cls.OUI_DB[clean_mac])
        ...
        if vendor:
            ...
            return OuiClassificationResult(
                vendor=vendor,
    ...
    def classify(
        self,
        device_dna: Union[DeviceDna, Dict[str, Any]],
    ) -> DeviceClassificationResult:
        result = (
            device_dna.model_dump()
            if isinstance(device_dna, DeviceDna)
            else dict(device_dna)
        )
        mac = str(result.get("mac", "")).replace(":", "").replace("-", "").upper()[:6]
        ...
        return DeviceClassificationResult.model_validate(result)
"""
GraphPath Unified Configuration Manager.
...
class ConfigurationManager:
    ...
    def __init__(self, profile_name: Optional[str] = None):
        self.profile_name = profile_name or os.environ.get("GRAPHPATH_PROFILE")
    ...
    def target_subnets(self) -> List[str]:
        env_subnets = os.environ.get("GRAPHPATH_TARGET_SUBNETS")
        if env_subnets:
    ...
    def get_ssh_credentials(self) -> Dict[str, str]:
        ...
        if os.environ.get("GRAPHPATH_SSH_USER"):
            creds["username"] = os.environ["GRAPHPATH_SSH_USER"]
        if os.environ.get("GRAPHPATH_SSH_PASS"):
            creds["password"] = os.environ["GRAPHPATH_SSH_PASS"]
        return creds

    def get_linux_ssh_credentials(self) -> Dict[str, str]:
        creds = dict(self.active_profile.get("linux_ssh", self.get_ssh_credentials()))
        if os.environ.get("GRAPHPATH_LINUX_SSH_USER"):
            creds["username"] = os.environ["GRAPHPATH_LINUX_SSH_USER"]
        if os.environ.get("GRAPHPATH_LINUX_SSH_PASS"):
            creds["password"] = os.environ["GRAPHPATH_LINUX_SSH_PASS"]
        return creds

    def get_snmp_community(self) -> str:
        return os.environ.get("GRAPHPATH_SNMP_COMMUNITY") or self.active_profile.get("snmp", {}).get("community", "public")
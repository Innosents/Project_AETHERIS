...
class TopologyStateManager:
    ...
    async def async_ttl_garbage_collector(self):
        ...
        while True:
            await asyncio.sleep(1) # Granular 1Hz sweep check
            current_time = time.monotonic()
            stale_threshold = current_time - self.TTL_SECONDS
            
            while self.ephemeral_nodes:
                # O(1) Front-of-Queue Inspection
                ...
                if oldest_payload["last_seen"] < stale_threshold:
                    ...
                    # The oldest node is still valid; all subsequent nodes are mathematically valid.
                    break 
...
async def cytoscape_delta_streamer(websocket, path, state_manager: TopologyStateManager):
    """
    WebSocket Daemon: Broadcasts the microscopic JSON delta strictly at 1Hz.
    Prevents Cytoscape cola/cose physics engine layout thrashing.
    """
    logging.info(f"Terra Digital DOM Socket Connected: {websocket.remote_address}")
    
    # 1. Initial Hydration: Send the entire static infrastructure baseline once.
    baseline = {"adds": list(state_manager.static_nodes.values()), "removes": []}
    await websocket.send(json.dumps(baseline))
    
    try:
        while True:
            await asyncio.sleep(1.0) # 1Hz Frame Rate
            delta_json = state_manager.extract_and_flush_deltas()
            
            # Null-check to prevent broadcasting empty `{adds: [], removes: []}` frames
            if len(json.loads(delta_json)["adds"]) > 0 or len(json.loads(delta_json)["removes"]) > 0:
                await websocket.send(delta_json)
    except websockets.exceptions.ConnectionClosed:
        logging.info("Frontend DOM Disconnected. Suspending broadcast.")

async def main():
    state_manager = TopologyStateManager()
    
    # Pre-load the immutable OT physics (Modicon, Mercury, AWG23 links)
    state_manager.static_nodes["PLC_01"] = {"data": {"id": "PLC_01", "label": "Modicon M221"}, "classes": ["ot_device", "ics_controller"]}
    
    # Spin up the concurrent O(K) garbage collector
    asyncio.create_task(state_manager.async_ttl_garbage_collector())
    
    # Bind the WebSocket Daemon
    async with websockets.serve(lambda ws, path: cytoscape_delta_streamer(ws, path, state_manager), "localhost", 8765):
        logging.info("AETHERIS Delta Matrix Bound to ws://localhost:8765")
        await asyncio.Future()  # Infinite lock
